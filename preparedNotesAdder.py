...
class FlashcardApp:
    def __init__(self,path,deck):
        ...
        # --- stopka ---
        btn_add = ttk.Button(footer, text="Dodaj fiszkę", command=lambda: self._on_add())
        btn_add.pack(side="left", padx=10, pady=10)

        btn_gpt = ttk.Button(footer, text="GPT translation", command=lambda: self._on_description_request())
        ...
        self.gemini_diff_val = tk.IntVar(footer, value=0) # Indicates whether gemini has different translation
        btn_gemini_diff = ttk.Checkbutton(footer, text="Gemini Diff", variable=self.gemini_diff_val)
        btn_gemini_diff.pack(side="right", padx=10, pady=10)
        
        self.to_edit_val = tk.IntVar(footer, value=0) # Indicates whether gemini has different translation
        btn_edit_val = ttk.Checkbutton(footer, text="To-edit", variable=self.to_edit_val)
    ...
    def _on_add(self):
        ...
        tags = []
        if self.image_val.get() == 1: tags.append("IMAGE")
        if self.error_val.get() == 1: tags.append("ERROR")
        if self.gemini_diff_val.get() == 1: tags.append("GEMINI_DIFF")
        if self.to_edit_val.get() == 1: tags.append("TO_EDIT")
        ...
        new_note_id = self.anki_api.add_flashcard(self.current_word, back_html, audio_string, self.deck, tags)
